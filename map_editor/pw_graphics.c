...
void PWG_draw_str(PWGraphicsContext *ctx, const char* str, int x, int y, int width, int height, int length){
...
int PWG_draw_image(PWGraphicsContext *ctx, PWSprite* s, int x, int y, int frame){
	...
	if(s->is_loop && s->is_animation){
		...
		sprite_str = s->sprite + s->height * s->width * (frame % s->animation_size);
	}
	
	for(j = 0; j < s->height && y + j < ctx->height; ++j){
		if(y + j < 0){
			screen_str += ctx->width;
			sprite_str += s->width;
			continue;
		}
		for(i = 0; i < s->width && x + i < ctx->width; ++i){
			if(x + i < 0){
				continue;
			}
			if(*sprite_str == 0){
			...
			//ctx->screen[(y + j) * ctx->width + x + i] = s->sprite[j * s->width + i];
